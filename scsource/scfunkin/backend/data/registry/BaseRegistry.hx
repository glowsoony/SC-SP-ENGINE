package scfunkin.backend.data.registry;

import scfunkin.utils.assets.DataAssets;
import scfunkin.utils.VersionUtil;
...
abstract class BaseRegistry<T:(IRegistryEntry<J> & Constructible<EntryConstructorFunction>), J>
{
  ...
  function printErrors(errors:Array<json2object.Error>, id:String = ''):Void
  {
    ...
    for (error in errors)
      scfunkin.backend.data.registry.DataError.printError(error);
  }
}
