...
class DeprecatedFunctions
{
  public static function implement(funk:FunkinLua)
  {
    ...
    funk.set("addAnimationByIndicesLoop", function(obj:String, name:String, prefix:String, indices:String, framerate:Int = 24) {
      ...
      return LuaUtil.addAnimByIndices(obj, name, prefix, indices, framerate, true);
    });
    ...
    funk.set("objectPlayAnimation", function(obj:String, name:String, forced:Bool = false, ?startFrame:Int = 0) {
      ...
      var spr:FlxSprite = Reflect.getProperty(LuaUtil.getTargetInstance(), obj);
    ...
    funk.set("setLuaSpriteCamera", function(tag:String, camera:String = '') {
      ...
      if (MusicBeatState.findVariableObj(tag))
      {
        MusicBeatState.variableMap(tag).get(tag).cameras = [LuaUtil.cameraFromString(camera)];
        return true;
      }
    ...
    funk.set("updateHitboxFromGroup", function(group:String, index:Int) {
      if (Std.isOfType(Reflect.getProperty(LuaUtil.getTargetInstance(), group), FlxTypedGroup))
      {
        Reflect.getProperty(LuaUtil.getTargetInstance(), group).members[index].updateHitbox();
        return;
      }
      Reflect.getProperty(LuaUtil.getTargetInstance(), group)[index].updateHitbox();
