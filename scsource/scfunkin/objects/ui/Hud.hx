...
import scfunkin.objects.ui.BarHit;
...
class Hud extends FlxGroup
{
  ...
  public dynamic function reloadColors()
  {
    ...
    for (i in [timeTxt, kadeEngineWatermark, scoreTxt, judgementCounter, botplayTxt])
    {
      ...
      if (i.color == scfunkin.utils.CoolUtil.colorFromString('0xFF000000')
        || i.color == scfunkin.utils.CoolUtil.colorFromString('#000000')
  ...
  public dynamic function displayPopedCombo(strumLine:StrumLine, note:Note, opponent:Bool = false):Void
  {
    ...
    else
    {
      ...
      {
        scfunkin.states.substates.ResultsScreenKadeSubstate.instance.registerHit(note, false, strumLine.cpuControlled, Rating.timingWindows[0].timingWindow);
      }
  ...
  {
    comboStats.updateAcc = scfunkin.utils.CoolUtil.floorDecimal(comboStats.ratingPercent * 100, 2);
