...
import scfunkin.backend.misc.CharacterOffsets;
...
import scfunkin.backend.scripting.psych.FunkinLua.LuaCamera;
import llua.*;
import llua.Lua;
#end
...
class LuaUtil
{
  ...
  public static function makeLuaCharacter(tag:String, character:String, isPlayer:Bool = false, flipped:Bool = false, characterType:String = 'OTHER')
  {
    ...
    if (MusicBeatState.getVariables("Character").get(tag) != null)
    {
      ...
      position = LuaUtil.getTargetInstance().members.indexOf(daChar);
    }
    ...
    LuaUtil.getTargetInstance().add(shit);

    if (position >= 0) // this should keep them in the same spot if they switch
    {
      LuaUtil.getTargetInstance().remove(shit, true);
      LuaUtil.getTargetInstance().insert(position, shit);
    }
